...
classesFile = cfgFolder + "/obj.names";
...
def postprocess(frame, outs, orgFrame):
    ...
    for i in indices:
        ...
        width = box[2]
        height = box[3]
        labelName = drawPred(classIds[i], confidences[i], left, top, left + width, top + height, orgFrame)
...
# 目前時間格式化成 20160320114539 形式 (2016-03-20 11:45:39)
...
if (args.image):
    ...
    outputFile = args.image[:-4] + '_' + now + '_yolo.jpg'
    outputTxt =  args.image[:-4] + '.txt'
    ...
    outputFile = args.video[:-4] + '_' + now + '_yolo.avi'
    outputTxt =  args.video[:-4] + '.txt'
...
while cv2.waitKey(1) < 0:
    ...
    labels = postprocess(frame, outs, orgFrame)
    ...
    if (args.image):
        ...
        if(outputToFile):
            ...
            if os.path.isfile(outputTxt):
                txtfile = open(outputTxt,'a')
                txtfile.write('\n')
                txtfile.write('\n'.join(labels))
            else:
                txtfile = open(outputTxt,'w')
                txtfile.write('\n'.join(labels))
            txtfile.close()
